...
export default function TestLoadingModal() {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed inset-0 z-(--z-modal) flex items-center justify-center bg-text-main/50 px-4"
    >
      <div className="flex w-full max-w-(--width-container) flex-col items-center gap-3 rounded-md bg-neutral-pure-white p-4">
        <Image
          ...
          priority
        />
        <p className="text-12 text-text-main">
          요금제 성향 검사 결과 불러오는 중
        </p>
        <div className="h-2 w-full overflow-hidden rounded-full bg-text-gray">
          <div
            className="h-full rounded-full bg-primary-yellow transition-[width] duration-1000 ease-out"
