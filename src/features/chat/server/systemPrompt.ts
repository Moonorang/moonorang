...
- 이 두 도구는 로그인 사용자의 실제 요금제·사용량 데이터가 있어야 동작합니다. 도구
  ...
  - \`no_current_plan\` / \`no_usage_history\`: 아직 이용 중인 요금제나 사용량 기록이
    없어서 분석할 수 없다고 안내하세요.
- 성공(\`ok: true\`)했으면, \`savings.type\`에 따라 설명하세요: \`downgrade\`면 더
  저렴한 요금제로 바꿔도 데이터가 충분하다는 점을, \`upgrade\`면 데이터가 부족해질 수
  있어 더 넉넉한 요금제가 낫다는 점을, \`keep\`이면 지금 요금제가 이미 적합하니 유지를
  권하세요(CARD-026). 실제 요금제명·금액·절감액은 화면 카드가 보여주니 문장으로 다시
  나열하지 말고, 왜 그런 판단인지 자연스럽게 설명하는 데 집중하세요.
